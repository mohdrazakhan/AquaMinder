  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        {/* ========== LEFT: Big Animated Water Tank ========== */}
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-6 flex flex-col items-center">
          <div className="flex items-center justify-between w-full mb-4">
            ...
            <span className={`text-xs font-bold px-2.5 py-1 rounded-lg border ${
              ...
              {isOnline ? "Live Monitoring" : "Last Known"}
            </span>
          </div>

          {/* Tank Container */}
          <div className="relative w-full max-w-[220px] mx-auto" style={{ height: 320 }}>
            {/* Inlet Pipe (top) */}
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={`w-3 h-3 rounded-full ${!isOnline ? "bg-slate-400" : isMotorOn ? "bg-emerald-500" : "bg-rose-500"}`} />
                <div>
                  <h3 className="text-base font-bold text-slate-900">Motor Pump</h3>
                  <p className="text-xs text-slate-500">{!isOnline ? "System Offline" : isMotorOn ? "Running" : "Standby"}</p>
                </div>
              </div>
              <div className="flex gap-2">
                ...
                >
                  {loading && activeCommand === "motor_on" ? "Sending..." : "ON"}
                </button>
                <button
                ...
                >
                  {loading && activeCommand === "motor_off" ? "Stopping..." : "OFF"}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={`w-3 h-3 rounded-full ${!isOnline ? "bg-slate-300" : isNightMode ? "bg-purple-600" : "bg-slate-300"}`} />
                <div>
                  <h3 className="text-base font-bold text-slate-900">Night Mode</h3>
                  <p className="text-xs text-slate-500">{!isOnline ? "System Offline" : isNightMode ? "Active — LEDs off" : "Inactive"}</p>
                </div>
              </div>
              <div className="flex gap-2">
                <button
          ...
          {/* Dry Run Protection */}
          <div className={`bg-white border rounded-xl p-5 ${isDryRunOverride ? "border-red-200" : "border-slate-200"}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-lg">{isDryRunOverride ? "🚨" : !isFlowSensorConnected ? "⚠️" : "🛡️"}</span>
                <div>
                  <h3 className="text-base font-bold text-slate-900">Dry Run Protection</h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {/* Flow Rate */}
            <div className={`border rounded-xl p-4 ${!isFlowSensorConnected ? "bg-slate-50 border-slate-200" : "bg-white border-slate-200"}`}>
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1">Flow Rate</span>
              <div className="flex items-baseline gap-1">
                <span className={`text-2xl font-bold tracking-tight ${!isFlowSensorConnected ? "text-slate-400" : "text-slate-900"}`}>{flowRate}</span>
                <span className="text-xs font-semibold text-sky-600">L/min</span>
              </div>
            </div>

            {/* Total Volume */}
            <div className="bg-white border border-slate-200 rounded-xl p-4">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1">Total Volume</span>
              <div className="flex items-baseline gap-1">
                <span className="text-2xl font-bold text-slate-900 tracking-tight">{totalVolume}</span>
                <span className="text-xs font-semibold text-sky-600">Liters</span>
              </div>
            </div>

            {/* Temperature */}
            <div className="bg-white border border-slate-200 rounded-xl p-4 col-span-2 sm:col-span-1">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1">Temperature</span>
              <div className="flex items-baseline gap-1">
                <span className="text-2xl font-bold text-slate-900 tracking-tight">{temperature ?? "—"}</span>
                {temperature && <span className="text-xs font-semibold text-sky-600">°C</span>}
              </div>
              {!temperature && <p className="text-[10px] text-slate-400 mt-0.5">Aqua Minder + only</p>}
            </div>
          </div>
        </div>
      </div>
